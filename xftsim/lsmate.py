...
def _solve_qap_ls(Y, Z, R, nb_threads=6, time_limit=30, tolerance=1e-5):
    ...
    tmp = np.argsort(np.apply_along_axis(np.mean,1,Y))[np.argsort(np.argsort(np.apply_along_axis(np.mean,1,Z)))]
    with localsolver.LocalSolver() as ls:
        ...
        qobj = model.sum(model.at(array_YY, p[i], p[j])*ZZ[i,j] for j in range(n) for i in range(n))
...
class KAssortativeMatingRegime(xft.mate.MatingRegime):
    ...
    def mate(self, 
        ...
        perm = _solve_qap_ls(female_components, male_components, self.cross_corr)
