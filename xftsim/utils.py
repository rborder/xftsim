...
class MixtureCount(VariableCount):
    def __init__(self, 
        ...
        def draw(n):
            choices = np.random.choice(len(componentCounts), size=n, replace=True, p=mixture_probabilities)
            output = np.zeros(n, dtype=int)
            for i in range(len(componentCounts)):
                output[choices==i] = componentCounts[i].draw(np.sum(choices==i))
            return output
        if all([component._expectation_impl for component in componentCounts]):
            expectation = np.sum([component.expectation * p for (component, p) in zip(componentCounts, mixture_probabilities)])
        else:
            expectation = None
        if all([component._mating_fraction_impl for component in componentCounts]):
            mating_fraction = np.sum([component._mating_fraction * p for (component, p) in zip(componentCounts, mixture_probabilities)])
        else:
...
def _standardize_array_hw(haplotypes, 
    ...
    for j in range(0,haplotypes.shape[1]//2):
        genotypes[:,j] = (haplotypes[:,2*j] + haplotypes[:,2*j+1] - means[j])/stds[j]
    return genotypes
...
def cov2cor(A):
    with np.errstate(invalid='ignore'):
        ...
        if isinstance(A, xr.DataArray):
            ...
            output = pd.DataFrame(S @ A @ S).set_index(A.index).T.set_index(A.index)
