...
class XftAccessor:
    ...
    def get_sample_indexer(self):
        if self._row_dim != 'sample': raise NotImplementedError
    ...
    def get_variant_indexer(self):
        if self._col_dim != 'variant': raise NotImplementedError
    ...
    def grep_component_index(self, keyword: str = 'phenotype'):
        if self._col_dim != 'component': raise TypeError
        pheno_cols= self._obj.component_name.values[self._obj.component_name.str.contains(keyword).values]
        component_index = self._obj.xft.get_component_indexer()[dict(component_name=pheno_cols)]
    ...
    def __getitem__(self, *args):
        ...
        if isinstance(argv, dict):
            row_dict = {key:value for (key,value) in argv.items() if key in self.row_vars}
            col_dict = {key:value for (key,value) in argv.items() if key in self.column_vars}
            ...
            column_indices = self.get_column_indexer()[col_dict].unique_identifier
        ## indexing with two postional args
            ...
            isvarindex = (isvarindex_1 or isvarindex_2) and (self._col_dim=='variant')
            iscomindex = (isinstance(argv[1], xft.index.ComponentIndex) and self._col_dim=='component')
            # print(argv)
